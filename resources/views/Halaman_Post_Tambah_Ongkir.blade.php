            ...
            <div class="container">
                <div class="row slider-text justify-content-center align-items-center">
                    ...
                    <div class="col-md-7 col-sm-12 text-center ftco-animate">
                        <h1 class="mb-3 mt-5 bread">Pilih Jasa Ongkir</h1>
                    </div>
        ...
        <div class="container">
            <div class="row">
                <div class="col-xl-12 ftco-animate">
                    <form action="/postpilihantambahongkir/{{ $dataorder->no_order }}" method="post" class="billing-form ftco-bg-dark p-3 p-md-5">
                        @csrf

                        <input type="hidden" name="origin" value="{{ $origin }}">
                        <input type="hidden" name="destination" value="{{ $destination }}">
                        <h3 class="mb-4 billing-heading">Pilih Jasa Ongkir</h3>
                        <div class="row align-items-end">
                            @foreach($cost as $cst)
                            <input type="hidden" name="courier" value="{{ $cst['name'] }}">
                            @foreach($cst['costs'] as $item)
                            <div class="col-md-3">
                                <div class="menu-entry">
                                    <div class="text text-center pt-4">
                                        ...
                                        @foreach($item['cost'] as $ongkir)
                                        <p class="price"><span>Rp. {{ $ongkir['value'] }}</span></p>
                                        <input type="hidden" name="total_ongkir" value="{{ $ongkir['value'] }}">

                                        <p class="estimasi">Estimasi Tiba <span>{{ $ongkir['etd'] }}</span> Hari</p>
                                        <input type="hidden" name="etd" value="{{ $ongkir['etd'] }}">

                                        @endforeach
                                        <p><button class="btn btn-white btn-outline-primary">Pilih</button></p>
                                    </div>
                                </div>
                            </div>
                            @endforeach
